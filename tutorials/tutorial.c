...
void drawRectWithOpacity(t_params *params, int x, int y, int width, int height, int color, float opacity) {
...
void drawRectBorder(t_params *params, int x, int y, int width, int height, int color, int thickness) {
    // Top border
...
void	renderMapGrid(t_params *params)
{
    ...
    drawRectWithOpacity(params, minimapX, minimapY, minimapWidth, minimapHeight, MINIMAP_BACKGROUND_COLOR, MINIMAP_OPACITY);
    ...
    for (int i = 0; i < MAP_NUM_ROWS; i++) {
        for (int j = 0; j < MAP_NUM_COLS; j++) {
            ...
            tileColor = getMapAt(i, j, params) != '0' ? MINIMAP_WALL_COLOR : MINIMAP_FLOOR_COLOR;

            // Draw tile with slight padding for grid effect
            drawRect(params, tileX+1, tileY+1, tileSize-2, tileSize-2, tileColor);
        }
    }

    // Draw border
    drawRectBorder(params, minimapX, minimapY, minimapWidth, minimapHeight, MINIMAP_BORDER_COLOR, 2);
}
...
void init_player(t_params *params)
{
    ...
    params->player.turnSpeed = 30.0 * (PI / 180); // Much faster rotation (8 degrees per frame)
...
{
	// WASD and Arrow keys for movement
	if ((keycode == KEY_W || keycode == KEY_UP) && params->player.walkDirection == 1)
		params->player.walkDirection = 0;
	if ((keycode == KEY_S || keycode == KEY_DOWN) && params->player.walkDirection == -1)
		params->player.walkDirection = 0;
	if ((keycode == KEY_A || keycode == KEY_LEFT) && params->player.turnDirection == -1)
		params->player.turnDirection = 0;
	if ((keycode == KEY_D || keycode == KEY_RIGHT) && params->player.turnDirection == 1)
		params->player.turnDirection = 0;
...
void movePlayer(t_params *params, float deltaTime)
{
    ...
    params->player.rotationAngle += params->player.turnDirection * params->player.turnSpeed * deltaTime;
    ...
    float moveStep = params->player.walkDirection * params->player.walkSpeed * deltaTime;
...
void castRay(float rayAngle, int stripId, t_params *params) {
    ...
    xintercept = params->player.x + (yintercept - params->player.y) / tan(rayAngle);
    ...
    while (isInsideMap(nextHorzTouchX, nextHorzTouchY)) {
        ...
        if (mapHasWallAt(xToCheck, yToCheck, params)) {
            ...
            horzWallTexture = getMapAt((int)floor(yToCheck / TILE_SIZE), (int)floor(xToCheck / TILE_SIZE), params);
    ...
    yintercept = params->player.y + (xintercept - params->player.x) * tan(rayAngle);
    ...
    while (isInsideMap(nextVertTouchX, nextVertTouchY)) {
        ...
        if (mapHasWallAt(xToCheck, yToCheck, params)) {
            ...
            vertWallTexture = getMapAt((int)floor(yToCheck / TILE_SIZE), (int)floor(xToCheck / TILE_SIZE), params);
    ...
    float horzHitDistance = foundHorzWallHit
        ? distanceBetweenPoints(params->player.x, params->player.y, horzWallHitX, horzWallHitY)
        : FLT_MAX;
    float vertHitDistance = foundVertWallHit
        ? distanceBetweenPoints(params->player.x, params->player.y, vertWallHitX, vertWallHitY)
        : FLT_MAX;
...
void renderMapRays(t_params *params) {
    ...
    for (int i = 0; i < NUM_RAYS; i += 60) {
        ...
        t_point start = {playerX, playerY};
        t_point end = {
            minimapX + params->rays[i].wallHitX * MINIMAP_SCALE_FACTOR,
            minimapY + params->rays[i].wallHitY * MINIMAP_SCALE_FACTOR
        };

        // Adjust alpha based on ray distance for a nice fading effect
        float distFactor = 1.0 - (params->rays[i].distance / (MAP_NUM_COLS * TILE_SIZE));
        rayAlpha *= distFactor > 0.2 ? distFactor : 0.2; // Don't let it fade completely
...
void renderWallProjection(t_params *params) {
    ...
    int wallColorVert = 0xAA0000;  // Dark red for vertical walls
    int wallColorHorz = 0x880000;  // Even darker red for horizontal walls

    for (int x = 0; x < NUM_RAYS; x++) {
        // Calculate perpendicular distance to avoid fisheye effect
        float perpDistance = params->rays[x].distance * cos(params->rays[x].rayAngle - params->player.rotationAngle);
        ...
        // Choose wall color based on whether hit was vertical or horizontal
        int wallColor = params->rays[x].wasHitVertical ? wallColorVert : wallColorHorz;
        ...
        for (int y = wallTopPixel; y < wallBottomPixel; y++) {
            put_pixel(params, x, y, wallColor);
        }
