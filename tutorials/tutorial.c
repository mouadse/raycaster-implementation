...
void renderMapGrid(t_params *params) {
  ...
  for (i = 0; i < MAP_NUM_ROWS; i++) {
    for (j = 0; j < MAP_NUM_COLS; j++) {
      ...
      tileColor = getMapAt(i, j, params) != '0' ? 0xFFFFFF : 0x000000;
      drawRect(params, tileX, tileY, TILE_SIZE * MINIMAP_SCALE_FACTOR,
...
void init_player(t_params *params) {
  ...
  params->player.turnSpeed = 50.0 * (PI / 180);
...
void renderMapPlayer(t_params *params) {
  // Draw player position on minimap as a small circle
  int playerX = params->player.x * MINIMAP_SCALE_FACTOR;
  int playerY = params->player.y * MINIMAP_SCALE_FACTOR;
  int playerSize = 8; // Size of player dot

  // Draw player dot
  for (int y = -playerSize / 2; y < playerSize / 2; y++) {
    for (int x = -playerSize / 2; x < playerSize / 2; x++) {
      if (x * x + y * y <= (playerSize / 2) * (playerSize / 2)) {
        put_pixel(params, playerX + x, playerY + y, 0xFF0000); // Red dot
      }
    }
  }
  ...
  // Draw direction line
  int lineLength = 20;
  int lineEndX = playerX + params->player.dx * lineLength;
  int lineEndY = playerY + params->player.dy * lineLength;

  t_point start = {playerX, playerY};
  t_point end = {lineEndX, lineEndY};

  draw_line(params, start, end, 0xFF0000);
}
...
void renderMapRays(t_params *params) {
  ...
  for (int i = 0; i < NUM_RAYS;
       i += 50) { // Draw every 50th ray for performance
    t_point start = {params->player.x * MINIMAP_SCALE_FACTOR,
                     params->player.y * MINIMAP_SCALE_FACTOR};
    t_point end = {params->rays[i].wallHitX * MINIMAP_SCALE_FACTOR,
                   params->rays[i].wallHitY * MINIMAP_SCALE_FACTOR};
    draw_line(params, start, end, 0xFFFF00); // Yellow color for rays
  }
}
