...
int main(void) {
  ...
  mlx = mlx_init();
  win = mlx_new_window(mlx, WIDTH, HEIGHT, "Half & Half Demo");
  img = mlx_new_image(mlx, WIDTH, HEIGHT);
  data = mlx_get_data_addr(img, &bpp, &line_size, &endian);

  // Fill the image - left half white, right half black
  for (y = 0; y < HEIGHT; y++) {
    for (x = 0; x < WIDTH; x++) {
      int pixel = y * line_size + x * (bpp / 8);
      int color = (x < WIDTH / 2) ? 0xFFFFFF : 0x000000;

      if (bpp == 32)
        *(unsigned int *)(data + pixel) = color;
      else if (bpp == 24) {
        data[pixel + 0] = (color >> 16) & 0xFF;
        data[pixel + 1] = (color >> 8) & 0xFF;
        data[pixel + 2] = color & 0xFF;
      }
    }
  }
...
// OLD CODE USED FOR TESTING MAPS and SOME MAP VALIDATION
...
//   const char *invalid_map2[] = {"111111", "10 001", "1000N1", "111111",
//   NULL};
